                                ...
                                @foreach($search_result as $job)
                                    <div class="job-show-content">
                                        <div class="row">
                                            <div class="col-lg-7">
                                                <div class="job-title">
                                                    @if(!empty($job->title))<h6 class="font-weight-bold"><a href="{{url('jobdetails/'.$job->id.'/'.$job->job_slug)}}" class="color-tax">{!!$job->title!!}</a> </h6>@endif
                                                </div>
                                                <?php $all_status = App\Models\ComInfoMatchingVisibility::where('job_id',$job->id)->get();
                                                    ...
                                                    @if(!empty($status->company_name) && $status->company_name == 1 )
                                                        @if(!empty($job->company_name))
                                                            @foreach($company_alt_name as $com_alt_name)  
                                                                ...
                                                                @else
                                                                <h6 class="font-weight-bold">{{$job->company_name}}</h6>
                                                                @endif
                                                            @endforeach
                                                        @endif
                                                    @endif
                                                ...
                                                @foreach($all_location as $loc)
                                                    @if($job->district_id == $loc->id)
                                                        @if(!empty($job->district_id))
                                                ...
                                                <div class="job-ex">
                                                @if(!empty($job->short_experience))
                                                    <h6> <i style="font-size:14px;" class="fa fa-briefcase" aria-hidden="true"></i> 
                                                        At least {{$job->short_experience}}
                                                        @if($job->short_experience > 1) 
                                                            years
                                                        @else 
                                                            year 
                                                        @endif      
                                                ...
                                                @if(!empty($job->min_salary) && !empty($job->max_salary))
                                                    <h6> <i style="font-size:14px;" class="fa fa-money"></i>  Tk. {{$job->min_salary}} - Tk. {{$job->max_salary}}
                                                    @if($job->salary == 0)
                                                            (Monthly)
                                                        @endif
                                                        @if($job->salary == 1)
                                            ...
                                            <div class="col-lg-5 d-flex align-items-end justify-content-end">
                                                ...
                                                <h6 class="m-0"><i style="font-size:14px;" class="fa fa-calendar" aria-hidden="true"></i> Deadline :
                                                    <?php $deadline =strtotime($job->deadline); echo  date(' jS F , Y', $deadline);?></h6>@endif                                        </div>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
                            @else
                                <h4 class="text-white bg-dark p-2 text-center mt-4 rounded">NO RESULT FOUND</h4>
                            @endif
                            @if(!empty($search_result))
                            <p>{{ $search_result->appends(request()->query())->links() }}</p>
                            @endif
                            </div>
                        </div>
                    </div>
