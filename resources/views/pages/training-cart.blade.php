...
<div class="container mt-5">
			...
			<?php $totalDiscount=0;?>
				...
				<?php $totalDiscount += (float)($row->price - $row->options['sale_price']) * $row->qty;?>
				<div class="row">
					<div class="col-2">
						<div class="about-image">
							<img src="{{asset('uploads/training/'.$row->options->image)}}" alt="" class="text-center pb-3" style="height: 100px;width: 100px">
						</div>
					</div>
					<div class="col-3">
					...
					<div class="col-2">
						<div class="about-text-content">
							...
							<input type="number" oninput="maxLengthCheck(this)" name="qty[]"  value="{{$row->qty}}" min="1" max="1" maxlength="3" required="" pattern="\d{3}" class="form-control">
					...
					<div class="col-1">
						<div class="about-text-content text-center">
							<a href="{{url('/remove/cart/'.$row->rowId)}}" class="text-danger h5 text-center" title="Remove item"><i class="fa fa-times-circle" aria-hidden="true"></i></a>
				...
				<button type="submit" class="float-left mr-4 btn btn-primary">Update Cart</button>
					...
					<a href="{{url('/training')}}" class="btn btn-primary float-left">Continue Shopping</a>
			<div class="row mt-5">
            <div class="col-7">
            </div>
				<div class="col-5 mt-5 float-left">
					...
					<a href="{{url('/train_checkout')}}" class="btn btn-primary mb-5 float-right">Checkout</a>
				</div>
			</div>
		</div>
</main>
